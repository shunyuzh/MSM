    ...
    def generate_question_vectors(self, questions: List[str]) -> T:
        ...
        with torch.no_grad():
            for j, batch_start in enumerate(range(0, n, bsz)):
                ...
                out = out.cpu()
                if args.similarity in ['cosine_scores', 'additive_margin']:
                    out = F.normalize(out)
...
def save_results_xorqa(
    ...
    for ids_and_scores, q_id, lang in zip(top_ids_and_scores, q_ids, languages):
        ...
        results.append({"id": q_id, "lang": lang, "ctxs" : ctxs})
...
def main(args):
    ...
    for in_file, out_file in zip(args.input_file, args.out_file):
        q_ids, questions, answers, languages = [], [], [], []
        if args.data == 'nq':  # get questions for NQ
            with open(in_file) as ifile:
                reader = csv.reader(ifile, delimiter='\t')
                for row in reader:
                    questions.append(row[0])
                    answers.append(eval(row[1]))
        elif args.data in ['mrtydi', 'mmarco']:  # get questions for Mr.Tydi
            with open(in_file) as ifile:
                reader = csv.reader(ifile, delimiter='\t')
                for row in reader:
                    q_ids.append(row[0])
        ...
        if args.data == 'nq':
            questions_doc_hits = validate(all_passages, answers, top_ids_and_scores, args.validation_workers, args.match)
            save_results_nq(all_passages, questions, answers, top_ids_and_scores, questions_doc_hits, out_file)
        elif args.data in ['mrtydi', 'mmarco']:
