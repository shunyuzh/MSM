...
from dpr.models.biencoder import (
    BiEncoder, 
    BiEncoderNllLoss, 
    CreateInputWithRandomCS, 
    CreateInputWithBatchCS, 
    BiEncoderBatch
)
...
class BiEncoderTrainer(object):
    ...
    def __init__(self, args):
        ...
        if args.use_cs is None:
            self.create_input = BiEncoder.create_biencoder_input
        elif args.use_cs == 'random':
            self.create_input = CreateInputWithRandomCS(args)
        elif args.use_cs == 'batch':
            self.create_input = CreateInputWithBatchCS(args)
        else:
    ...
    def validate_and_save(self, epoch: int, iteration: int, scheduler, data_iterator: MultiSetDataIterator):
        ...
        if save_cp:
            ...
            elif validation_loss < (self.best_validation_result or validation_loss + 1):
    ...
    def validate_nll(self, data_iterator) -> float:
        ...
        for i, samples_batch in enumerate(data_iterator.iterate_data()):
            ...
            loss, correct_cnt = _do_biencoder_fwd_pass(self.biencoder, biencoder_input, self.tensorizer, args, train=False)
    ...
    def validate_average_rank(self, data_iterator) -> float:
        ...
        sim_score_f = BiEncoderNllLoss(args).eval_similarity_function
        q_represenations = []
        ctx_represenations = []
    ...
    def _save_checkpoint(self, scheduler, epoch: int, offset: int) -> str:
        ...
        if args.enable_marco:
            if epoch == 2:
                ...
                cp = os.path.join(args.output_dir, 'model{}.pt'.format(epoch))
...
def _calc_loss(args, loss_function, local_q_vector, local_ctx_vectors, local_positive_idxs,
               local_hard_negatives_idxs: list = None, train: bool = True):
    """
    ...
    loss, is_correct = loss_function.calc(global_q_vector, global_ctxs_vector, positive_idx_per_question, hard_negatives_per_question, train)

    return loss, is_correct


def _do_biencoder_fwd_pass(model: nn.Module, input: BiEncoderBatch, tensorizer: Tensorizer, args, train: bool = True) -> Tuple[torch.Tensor, int]:
    ...
    loss, is_correct = _calc_loss(args, loss_function, local_q_vector, local_ctx_vectors, input.is_positive, input.hard_negatives, train)
...
def main():
    ...
    # code switch
    parser.add_argument("--use_cs", default=None, type=str, help="whether to use code switch")
    parser.add_argument("--cs_langs", default=None, type=str, nargs='+', help="switch languages")
    parser.add_argument("--sentence_thresh", default=1, type=float, help="threshold for random code switch")
    parser.add_argument("--token_thresh", default=1, type=float, help="threshold for random code switch")
    parser.add_argument("--batch_thresh", default=0.5, type=float, help="threshold for batch code switch")

    # similarity
    parser.add_argument("--margin", default=0.3, type=float, help="margin for additive margin softmax loss")
    parser.add_argument("--scale", default=20, type=float, help="re-scale length")
    ...
    parser.add_argument("--enable_marco", action='store_true',
                        help="train on marco and save all checkpoints")
