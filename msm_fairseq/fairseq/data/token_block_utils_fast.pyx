...
@cython.wraparound(False)
@cython.nonecheck(False)
cpdef np.ndarray[DTYPE_t, ndim=2] _get_slice_indices_fast2(np.ndarray[DTYPE_t, ndim=1] sizes, str break_mode, int block_size, int document_sep_len, int doc_sentences, int doc_sentences_low):
    cdef DTYPE_t tok_idx = 0
    cdef DTYPE_t sz_idx = 0
    cdef DTYPE_t curr_size = 0
    cdef DTYPE_t sentences_number = 0
    cdef DTYPE_t i = 0
    cdef DTYPE_t length
    cdef DTYPE_t total_size
    cdef DTYPE_t[:] sizes_view = sizes
    ...
    if break_mode is None or break_mode == 'none':
        ...
        while sz_idx < len(sizes_view):
            if curr_size + sizes_view[sz_idx] <= block_size or curr_size == 0:
                ...
                curr_size = 0
                sentences_number += 1
                # finish loading a single block
                if sentences_number == doc_sentences:
                    sentences_number = 0

            # when meet document end
            if sizes_view[sz_idx] == document_sep_len:
                slice_indices_list.append((tok_idx, tok_idx + curr_size))
                tok_idx += curr_size
                curr_size = 0
                sentences_number += 1
                if sentences_number < doc_sentences_low:
                    del slice_indices_list[-sentences_number: ]
                else:
                    while sentences_number < doc_sentences:
                        slice_indices_list.append((tok_idx, tok_idx + sizes_view[sz_idx]))
                        sentences_number += 1
                tok_idx += sizes_view[sz_idx]
                sz_idx += 1
                sentences_number = 0
    ...
    elif break_mode == 'doc_block2':
        # not enable, reserved for debug
        while sz_idx < len(sizes_view):
            if (
                (curr_size + sizes_view[sz_idx] <= block_size or curr_size == 0)
                # an empty sentence indicates end-of-document:
