...
class Trainer(object):
    ...
    def train_step(self, samples, dummy_batch=False, raise_oom=False):
        ...
        for i, sample in enumerate(samples):
            # sample sentences to file
            if self.args.gather_log_dir is not None:
                # print("train sample \n", sample)
                doc_data = {"task": sample['net_input']['task'].numpy().tolist(),
                            "lg": sample['index'].numpy().tolist(),
                            "doc_data": sample['net_input']['document_model']['src_tokens'][:, :5].numpy().tolist(),
                            "mlm_data": sample['net_input']['mlm']['src_tokens'][:, :5].numpy().tolist(),
                            }
                # print("train sample \n", doc_data)
                with open(os.path.join(self.args.gather_log_dir, "train_step_{}.json".format(self.args.device_id)), 'a+') as f:
            ...
            try:
                with maybe_no_sync():
                    ...
                    if self.args.enable_momentum_update:
                        if len(self.criterion.moco_model) == 0:
                            print("create moco model")
                            self.criterion.moco_model = [copy.deepcopy(self.model)]
                            self.criterion.moco_model[0].train()
                            for parameter in self.criterion.moco_model[0].parameters():
        ...
        try:
            ...
            self.optimizer.step()
            if self.args.enable_momentum_update:
                m = self.args.momentum_update_rate
                for param_aux, param in zip(self.criterion.moco_model[0].parameters(), self.model.parameters()):
                    param_aux.data = (1.0 - m) * param.data + m * param_aux.data
