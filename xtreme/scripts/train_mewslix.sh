...
TYPE=${2:-bert-base-multilingual-cased}
EXP=${3:-expname}
GPU=${4:-0}
...
if [ $TYPE == "bert-base-multilingual-cased" ]; then
...
# using the current settings.
elif [ $TYPE == "xlm-roberta-large" ]; then
  ...
  SAVE_STEPS=2000
# END GOOGLE-INTERNAL
elif [ $TYPE == "xlm-roberta-base" ]; then
  MODEL_TYPE="xlmr-retrieval"
  LR=2e-5
  DO_LOWER_CASE="--do_lower_case"
  PER_GPU_BATCH_SIZE=16  # largest power of two that fit 16GB GPU RAM
  LOGGING_STEPS=100
  SAVE_STEPS=200
else
...
MODEL_DIR="${OUT_DIR}/${TASK}/${EXP}_${HYPER_INFO}"
mkdir -p $MODEL_DIR
