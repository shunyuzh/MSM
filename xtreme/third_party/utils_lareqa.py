...
def mean_avg_prec_at_k(question_set, candidate_set, k=None, file=None):
  ...
  lgs = 'ar de el en es hi ru th tr vi zh'.split()
  eval_results = []
  for lg in lgs:
    all_questions = [q for q in question_set.as_list() if q.language == lg]
    ...
    for embedding_type in ['sentences_and_contexts']:
      ...
      print( lg + ": " + str(np.mean(ap_scores)))
      eval_results.append(np.around(np.mean(ap_scores), 6))

  print("avg: " + str(np.mean(eval_results)))
  eval_results.append(np.around(np.mean(eval_results), 6))
  with open(file, "w") as f:
    f.write('ar de el en es hi ru th tr vi zh avg' + '\n')
    f.write(' '.join(list(map(str, eval_results)))+ "\n")